...
关节角保存为当前下拉任务的 HomePose。记录前必须先停止对应机械臂；记录本身
不会驱动机械臂，确认对话框也会提示下一次 Home 才会运动。首次创建持久化配置
时只有 `粉末称量` 继承旧 Home；另两个任务在左右臂都完成记录前会拒绝 Home。
...
Home，否则拒绝执行。位姿持久化在 `${TELEOP_DATA_ROOT}/operator_gui/poses.yaml`。
