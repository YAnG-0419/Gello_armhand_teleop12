...
class LinkerHandApi:
    def __init__(
        self, hand_type="left", hand_joint="L10", modbus="None", can="can0"
        ...
        if version == None or len(version) == 0:
            ColorMsg(
                msg="Warning: Hardware version number not recognized, it is recommended to terminate the program and re insert USB to CAN conversion",
                color="yellow",
            )
        else:
    ...
    def finger_move(self, pose=[]):
        ...
        if any(not isinstance(x, (int, float)) or x < 0 or x > 255 for x in pose):
            ColorMsg(
                msg=f"The numerical range cannot be less than 0 or greater than 255",
                color="red",
            )
            return
        if (self.hand_joint.upper() == "O6" or self.hand_joint.upper() == "L6") and len(
            pose
        ) == 6:
            ...
            ColorMsg(
                msg=f"Current LinkerHand is {self.hand_type}{self.hand_joint}, action sequence is {pose}, does not match",
                color="red",
            )
    ...
    def set_speed(self, speed=[100] * 5):
        """# Set speed"""
        has_non_int = any(
            not isinstance(x, (int, float)) or x < 0 or x > 255 for x in speed
        )
        if has_non_int:
            print(
                "Set Speed The numerical range can only be positive integers or floating-point numbers between 0 and 255",
                flush=True,
            )
            return
        if len(speed) < 5:
        ...
        if self.hand_joint == "L7" and len(speed) < 7:
            ...
            return
        ColorMsg(
            msg=f"{self.hand_type} {self.hand_joint} set speed to {speed}",
            color="green",
        )
    ...
    def set_joint_speed(self, speed=[100] * 5):
        ...
        if any(not isinstance(x, (int, float)) or x < 10 or x > 255 for x in speed):
            ColorMsg(
                msg=f"The numerical range cannot be less than 10 or greater than 255",
                color="red",
            )
            return
        self.hand.set_speed(speed=speed)
    ...
    def set_torque(self, torque=[180] * 5):
        """Set maximum torque"""
        has_non_int = any(
            not isinstance(x, (int, float)) or x < 0 or x > 255 for x in torque
        )
        if has_non_int:
            print(
                "Set Torque The numerical range can only be positive integers or floating-point numbers between 0 and 255",
                flush=True,
            )
            return
        if len(torque) < 5:
        ...
        if (self.hand_joint == "L6" or self.hand_joint == "O6") and len(torque) != 6:
            ...
            return
        ColorMsg(
            msg=f"{self.hand_type} {self.hand_joint} set maximum torque to {torque}",
            color="green",
        )
    ...
    def set_current(self, current=[250] * 5):
        ...
        if any(not isinstance(x, (int, float)) or x < 0 or x > 255 for x in current):
            print(
                "Set Current The numerical range can only be positive integers or floating-point numbers between 0 and 255",
                flush=True,
            )
            return
        if self.hand_joint == "L20":
