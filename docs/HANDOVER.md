...
- Standard startup is `docker compose up franka-control teleop-control moveit-ik preset-ik gello-bridge hand-control orbbec orbbec-305`, then `ops/run/run_teleop.sh`, then `python apps/operator_gui/operator_gui.py`. `moveit-ik` is read-only and starts no controller manager. `preset-ik` is the warm TCP IK helper for Q/W/E. VIVE and PICO remain optional fallbacks; never run multiple UDP bridges.
