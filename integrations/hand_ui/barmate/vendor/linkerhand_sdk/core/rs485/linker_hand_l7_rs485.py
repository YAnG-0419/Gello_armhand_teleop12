...
class LinkerHandL7RS485:
    ...
    def __init__(
        ...
        if not self.connected:
            raise ConnectionError(
                f"RS485 connect fail to {modbus_port} with ID {hex(hand_id)}."
            )
    ...
    def _read_input_registers(self, address: int, count: int) -> List[int]:
        ...
        try:
            ...
            if rsp.isError():
                raise RuntimeError(
                    f"Modbus FC04 读取失败。地址: {address}, 错误: {rsp}"
                )
    ...
    def _write_holding_registers(self, address: int, values: List[int]):
        ...
        # 单个写入 (FC 06)
            ...
            write_func = lambda address, values, slave: self.cli.write_register(
                address, values[0], slave
            )
        else:
            raise ValueError("写入值列表不能为空。")

        try:
            rsp = write_func(address=address, values=values, slave=self.slave)

            if rsp.isError():
                raise RuntimeError(
                    f"Modbus FC16 写入失败。地址: {address}, 错误: {rsp}"
                )
    ...
    def get_joint_positions(self) -> Dict[str, int]:
        """读取当前关节位置 (地址 0-6)。"""
        registers = self._read_input_registers(
            IR_ADDR["Current_Position_Start"], _JOINT_COUNT
        )
    ...
    def get_current_torques(self) -> Dict[str, int]:
        """读取当前关节转矩 (地址 7-13)。"""
        registers = self._read_input_registers(
            IR_ADDR["Current_Torque_Start"], _JOINT_COUNT
        )
        # return dict(zip(O7_JOINT_KEYS, registers))
        return registers
    ...
    def get_current_speeds(self) -> Dict[str, int]:
        """读取当前关节速度 (地址 14-20)。"""
        registers = self._read_input_registers(
            IR_ADDR["Current_Speed_Start"], _JOINT_COUNT
        )
        # return dict(zip(O7_JOINT_KEYS, registers))
        return registers
    ...
    def get_temperatures(self) -> Dict[str, int]:
        """读取当前关节温度 (地址 21-27)。"""
        registers = self._read_input_registers(
            IR_ADDR["Current_Temperature_Start"], _JOINT_COUNT
        )
        # return dict(zip(O7_JOINT_KEYS, registers))
        return registers
    ...
    def get_error_codes(self) -> Dict[str, int]:
        """读取当前关节错误码 (地址 28-34)。"""
        registers = self._read_input_registers(
            IR_ADDR["Error_Code_Start"], _JOINT_COUNT
        )
        # return dict(zip(O7_JOINT_KEYS, registers))
        return registers
    ...
    def get_pressure_matrix(self, finger_id: int) -> np.ndarray:
        ...
        registers_16bit: List[int] = self._read_input_registers(
            IR_ADDR["Pressure_Data_Start"], _PRESSURE_REG_COUNT
        )
    ...
    def set_joint_positions(self, joint_angles: List[int]):
        ...
        if len(joint_angles) != _JOINT_COUNT:
            raise ValueError(
                f"需要 {_JOINT_COUNT} 个关节位置值，提供了 {len(joint_angles)} 个。"
            )
    ...
    def set_torques(self, torques: List[int]):
        ...
        if len(torques) != _JOINT_COUNT:
            raise ValueError(
                f"需要 {_JOINT_COUNT} 个关节转矩值，提供了 {len(torques)} 个。"
            )
    ...
    def set_speeds(self, speeds: List[int]):
        ...
        if len(speeds) != _JOINT_COUNT:
            raise ValueError(
                f"需要 {_JOINT_COUNT} 个关节速度值，提供了 {len(speeds)} 个。"
            )
