...
class LinkerHandO6RS485:
    ...
    def _execute_write(self, address: int, values: List[int]):
        ...
        rsp = self.cli.write_registers(address=address, values=values, slave=self._id)
    ...
    def get_thumb_pitch(self) -> int:
        return self._read_reg(REG_RD_CURRENT_THUMB_PITCH)

    def get_thumb_yaw(self) -> int:
        return self._read_reg(REG_RD_CURRENT_THUMB_YAW)

    def get_index_pitch(self) -> int:
        return self._read_reg(REG_RD_CURRENT_INDEX_PITCH)

    def get_middle_pitch(self) -> int:
        return self._read_reg(REG_RD_CURRENT_MIDDLE_PITCH)

    def get_ring_pitch(self) -> int:
        return self._read_reg(REG_RD_CURRENT_RING_PITCH)

    def get_little_pitch(self) -> int:
        return self._read_reg(REG_RD_CURRENT_LITTLE_PITCH)

    def get_thumb_torque(self) -> int:
        return self._read_reg(REG_RD_CURRENT_THUMB_TORQUE)

    def get_thumb_yaw_torque(self) -> int:
        return self._read_reg(REG_RD_CURRENT_THUMB_YAW_TORQUE)

    def get_index_torque(self) -> int:
        return self._read_reg(REG_RD_CURRENT_INDEX_TORQUE)

    def get_middle_torque(self) -> int:
        return self._read_reg(REG_RD_CURRENT_MIDDLE_TORQUE)

    def get_ring_torque(self) -> int:
        return self._read_reg(REG_RD_CURRENT_RING_TORQUE)

    def get_little_torque(self) -> int:
        return self._read_reg(REG_RD_CURRENT_LITTLE_TORQUE)

    def get_thumb_speed(self) -> int:
        return self._read_reg(REG_RD_CURRENT_THUMB_SPEED)

    def get_thumb_yaw_speed(self) -> int:
        return self._read_reg(REG_RD_CURRENT_THUMB_YAW_SPEED)

    def get_index_speed(self) -> int:
        return self._read_reg(REG_RD_CURRENT_INDEX_SPEED)

    def get_middle_speed(self) -> int:
        return self._read_reg(REG_RD_CURRENT_MIDDLE_SPEED)

    def get_ring_speed(self) -> int:
        return self._read_reg(REG_RD_CURRENT_RING_SPEED)

    def get_little_speed(self) -> int:
        return self._read_reg(REG_RD_CURRENT_LITTLE_SPEED)

    def get_thumb_temp(self) -> int:
        return self._read_reg(REG_RD_THUMB_TEMP)

    def get_thumb_yaw_temp(self) -> int:
        return self._read_reg(REG_RD_THUMB_YAW_TEMP)

    def get_index_temp(self) -> int:
        return self._read_reg(REG_RD_INDEX_TEMP)

    def get_middle_temp(self) -> int:
        return self._read_reg(REG_RD_MIDDLE_TEMP)

    def get_ring_temp(self) -> int:
        return self._read_reg(REG_RD_RING_TEMP)

    def get_little_temp(self) -> int:
        return self._read_reg(REG_RD_LITTLE_TEMP)

    def get_thumb_error(self) -> int:
        return self._read_reg(REG_RD_THUMB_ERROR)

    def get_thumb_yaw_error(self) -> int:
        return self._read_reg(REG_RD_THUMB_YAW_ERROR)

    def get_index_error(self) -> int:
        return self._read_reg(REG_RD_INDEX_ERROR)

    def get_middle_error(self) -> int:
        return self._read_reg(REG_RD_MIDDLE_ERROR)

    def get_ring_error(self) -> int:
        return self._read_reg(REG_RD_RING_ERROR)

    def get_little_error(self) -> int:
        return self._read_reg(REG_RD_LITTLE_ERROR)

    def get_hand_freedom(self) -> int:
        return self._read_reg(REG_RD_HAND_FREEDOM)

    def get_hand_version(self) -> int:
        return self._read_reg(REG_RD_HAND_VERSION)

    def get_hand_number(self) -> int:
        return self._read_reg(REG_RD_HAND_NUMBER)

    def get_hand_direction(self) -> int:
        return self._read_reg(REG_RD_HAND_DIRECTION)

    def get_software_version(self) -> int:
        return self._read_reg(REG_RD_SOFTWARE_VERSION)

    def get_hardware_version(self) -> int:
        return self._read_reg(REG_RD_HARDWARE_VERSION)
    ...
    def set_thumb_pitch(self, v: int):
        self._write_reg(REG_WR_THUMB_PITCH, v)

    def set_thumb_yaw(self, v: int):
        self._write_reg(REG_WR_THUMB_YAW, v)

    def set_index_pitch(self, v: int):
        self._write_reg(REG_WR_INDEX_PITCH, v)

    def set_middle_pitch(self, v: int):
        self._write_reg(REG_WR_MIDDLE_PITCH, v)

    def set_ring_pitch(self, v: int):
        self._write_reg(REG_WR_RING_PITCH, v)

    def set_little_pitch(self, v: int):
        self._write_reg(REG_WR_LITTLE_PITCH, v)

    def set_thumb_torque(self, v: int):
        self._write_reg(REG_WR_THUMB_TORQUE, v)

    def set_thumb_yaw_torque(self, v: int):
        self._write_reg(REG_WR_THUMB_YAW_TORQUE, v)

    def set_index_torque(self, v: int):
        self._write_reg(REG_WR_INDEX_TORQUE, v)

    def set_middle_torque(self, v: int):
        self._write_reg(REG_WR_MIDDLE_TORQUE, v)

    def set_ring_torque(self, v: int):
        self._write_reg(REG_WR_RING_TORQUE, v)

    def set_little_torque(self, v: int):
        self._write_reg(REG_WR_LITTLE_TORQUE, v)

    def set_thumb_speed(self, v: int):
        self._write_reg(REG_WR_THUMB_SPEED, v)

    def set_thumb_yaw_speed(self, v: int):
        self._write_reg(REG_WR_THUMB_YAW_SPEED, v)

    def set_index_speed(self, v: int):
        self._write_reg(REG_WR_INDEX_SPEED, v)

    def set_middle_speed(self, v: int):
        self._write_reg(REG_WR_MIDDLE_SPEED, v)

    def set_ring_speed(self, v: int):
        self._write_reg(REG_WR_RING_SPEED, v)

    def set_little_speed(self, v: int):
        self._write_reg(REG_WR_LITTLE_SPEED, v)
    ...
    def set_joint_positions(self, joint_angles: List[Any] = None):
        ...
        if not self.is_valid_6xuint8(joint_angles):
            ...
            raise ValueError(
                "Joint angles must be a list of 6 values between 0 and 255 (convertible to int)."
            )
    ...
    def set_speed(self, speed: List[Any] = None):
        ...
        if not self.is_valid_6xuint8(speed):
            ...
            raise ValueError(
                "Speed values must be a list of 6 values between 0 and 255 (convertible to int)."
            )
    ...
    def set_torque(self, torque: List[Any] = None):
        ...
        if not self.is_valid_6xuint8(torque):
            ...
            raise ValueError(
                "Torque values must be a list of 6 values between 0 and 255 (convertible to int)."
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="O6 Hand Modbus tester (using pymodbus 3.5.1)"
    )
    ...
    try:
        ...
        with LinkerHandO6RS485(
            hand_id=args.hand_id, modbus_port=args.port, baudrate=115200
        ) as hand:
            hand.dump_status()
            print("执行 relax → 伸直")
            hand.relax()
