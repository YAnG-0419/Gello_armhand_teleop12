...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="pico.yaml to compare against and optionally rewrite",
    )
    parser.add_argument(
    ...
    try:
        updated = replace_serials(config_text, mapping)
    except CalibrationError as error:
    ...
    # The stricter of the two validators that consume this file; a broken
    # write must fail here, not at the next teleop start.
    from pico_bimanual_franka_teleop.config import load_config

    load_config(args.config)
    print(f"Wrote and re-validated {args.config}.")
    return 0
