...
class LeftWujiDatasetRecorder:
    ...
    A JSONL sidecar is flushed throughout the run, so samples remain recoverable
    if the process cannot perform its normal NPZ finalization.
    """
    ...
    def record(
        ...
        *,
        ...
        quaternion_xyzw = np.asarray(
            pin.Quaternion(rotation).coeffs(), dtype=np.float64
        )
    ...
    def _write_loop(self) -> None:
        try:
            with self.raw_output.open("x", encoding="utf-8") as raw:
                ...
                raw.flush()
                os.fsync(raw.fileno())
        except BaseException as error:  # keep failures out of the control loop
            self._failed = error
    ...
    def _save_npz(self) -> None:
        ...
        arrays = {
            ...
            "hand_stale_timeout_s": np.asarray(
                self.hand_stale_timeout_s, dtype=np.float64
            ),
            "dropped_samples": np.asarray(self._dropped, dtype=np.int64),
            "wall_time_ns": np.asarray(
                [row["wall_time_ns"] for row in rows], dtype=np.int64
            ),
            "monotonic_time_s": np.asarray(
                [row["monotonic_time_s"] for row in rows], dtype=np.float64
            ),
            "arm_q": np.asarray(
                [row["arm_q"] for row in rows], dtype=np.float64
            ).reshape(-1, 7),
            "hand_q": np.asarray(
                [row["hand_q"] for row in rows], dtype=np.float64
            ).reshape(-1, 20),
            "hand_sample_time_s": np.asarray(
                [row["hand_sample_time_s"] for row in rows], dtype=np.float64
            ),
            "hand_age_s": np.asarray(
                [row["hand_age_s"] for row in rows], dtype=np.float64
            ),
            "hand_valid": np.asarray(
                [row["hand_valid"] for row in rows], dtype=np.bool_
            ),
            "ee_position": np.asarray(
                [row["ee_position"] for row in rows], dtype=np.float64
            ).reshape(-1, 3),
            "ee_quaternion_xyzw": np.asarray(
                [row["ee_quaternion_xyzw"] for row in rows], dtype=np.float64
            ).reshape(-1, 4),
            "arm_active": np.asarray(
                [row["arm_active"] for row in rows], dtype=np.bool_
            ),
            "hand_active": np.asarray(
                [row["hand_active"] for row in rows], dtype=np.bool_
            ),
        }
    ...
    def close(self) -> None:
        ...
        if self._failed is not None:
            raise RuntimeError(
                f"dataset raw writer failed: {self._failed}"
            ) from self._failed
        self._save_npz()
