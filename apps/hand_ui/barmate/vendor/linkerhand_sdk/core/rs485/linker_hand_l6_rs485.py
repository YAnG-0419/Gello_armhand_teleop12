...
class LinkerHandL6RS485:
    ...
    def _read_input_registers(self, address: int, count: int) -> List[int]:
        ...
        result = self.cli.read_input_registers(
    ...
    def _write_registers(self, address: int, values: List[int]):
        """写入多个寄存器"""
        time.sleep(_INTERVAL)
        result = self.cli.write_registers(
            address=address, values=values, slave=self.slave
        )
        if result.isError():
            raise RuntimeError(
                f"写入多个寄存器失败: address={address}, values={values}"
            )
    ...
    def _pressure(self, finger: int) -> np.ndarray:
        ...
        # 1. 写入手指选择寄存器 (地址 60)
        time.sleep(0.008)
        wrsp = self.cli.write_register(
            address=write_address, value=finger_write_value, slave=self.slave
        )
        if wrsp.isError():
            raise RuntimeError(
                f"写入手指选择 {finger} 到地址 {write_address} 失败: {wrsp}"
            )
        ...
        # 2. 读取地址 62 的数据
        rrsp = self.cli.read_input_registers(
            address=read_address, count=read_count, slave=self.slave
        )
    ...
    def dump_status(self):
        ...
        try:
            ...
            for i, name in enumerate(self.JOINT_NAMES):
                print(
                    f"  {name:15s}: 角度={angles[i]:3d}, 扭矩={torques[i]:3d}, "
...
if __name__ == "__main__":
    # 使用示例
    try:
        with LinkerHandL6RS485(
            hand_id=0x27, modbus_port="/dev/ttyUSB0", baudrate=115200
        ) as hand:
            print("连接成功!")
