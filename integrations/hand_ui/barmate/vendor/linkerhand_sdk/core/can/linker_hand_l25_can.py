...
class FrameProperty(Enum):
    ...
    ROOT1_POS = (
        0x03  # Root1 joint position | The root joint closest to the palm [0,1,2,3,4]
    )
    ROOT2_POS = 0x04  # Root2 joint position | The root joint closest to the palm [15, 16,17,18,19]
    ROOT3_POS = (
        0x05  # Root3 joint position | The root joint closest to the palm Not available
    )
    ...
    ROOT1_TEMPERATURE = (
        0x23  # Root1 joint temperature | The root joint closest to the palm
    )
    ROOT2_TEMPERATURE = (
        0x24  # Root2 joint temperature | The root joint closest to the palm
    )
    ROOT3_TEMPERATURE = (
        0x25  # Root3 joint temperature | The root joint closest to the palm
    )
    TIP_TEMPERATURE = (
        0x26  # Fingertip joint temperature | The root joint closest to the palm
    )
    # Parallel command area
...
class LinkerHandL25Can:
    ...
    def joint_map_2(self, pose):
        l25_pose = (
            [0.0] * 30
        )  # L25 CAN默认接收30个数据 pose控制L25发送的指令数据默认25个，这里进行映射
        """
        需要进行映射
        # L25 CAN数据格式
