...
docker compose \
  ...
  python3 /workspace/franka_upper_body_teleop/powderweighing/calibrate_poses.py \
  --device 1
```
...
数据写入 `powderweighing/poses.json`。所有退出路径都会将右手停回 URDF
neutral。

## 后续重新调整 pose

不需要修改策略代码或重新编译。重新运行交互标定程序并保存，就会覆盖
`poses.json` 中的 `pitch_ready` 和 `pitch`。
...
3. 调好 `pitch_ready` 后按 `s`，调好 `pitch` 后再按一次 `s`。第二次保存
   会原子更新 `powderweighing/poses.json`。按 `q` 或 Ctrl-C 会放弃本次
   修改并将右手停回 neutral。

默认从右手当前反馈开始。如果只想在已有 `pitch_ready` 上小幅精调，可以
把 `poses.json` 中最新的 `pitch_ready` 数组传给 `--seed-ticks`。例如当前
这版为：
...
docker compose \
  ...
  python3 /workspace/franka_upper_body_teleop/powderweighing/calibrate_poses.py \
  --device 1 \
  --seed-ticks 93,184,71,108,92,140,171,63,219,255,230,182,150,248,247,101,77,186,201,0
```
...
./scripts/run_operator.sh \
  --right-hand-strategy-config powderweighing/poses.json
```
...
```bash
cd ~/llx/gello_upper_body_teleop
python powderweighing/run_powderweighing.py \
  --config powderweighing/poses.json \
