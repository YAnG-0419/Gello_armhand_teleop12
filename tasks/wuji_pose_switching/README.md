...
This optional task policy uses two mappings saved by `apps/wuji_ui`:

- left distance trigger `11` selects pose `神勺准备`;
- left composite trigger `22` selects pose `9999`.
...
`READY` and commands `神勺准备`. While in `READY`, holding composite trigger
`22` above its entry fraction and dwell moves to `ACTIVE` and commands `9999`.

Dropping below trigger `22`'s exit fraction returns `ACTIVE` to `READY`.
Opening beyond trigger `11`'s exit distance returns either fixed-pose state to
...
gesture thresholds. Relearning `11` or `22` in the UI therefore updates the
