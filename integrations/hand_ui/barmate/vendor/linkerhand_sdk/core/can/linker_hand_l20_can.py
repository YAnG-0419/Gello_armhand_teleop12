...
class FrameProperty(Enum):
    ...
    JOINT_ROLL_R = (
        0x03  # Short frame roll angle - only used for thumb | Returns this type of data
    )
    JOINT_TIP_R = (
        0x04  # Short frame fingertip angle control | Returns this type of data
    )
    ...
    JOINT_PITCH_NR = (
        0x11  # Pitch angle - finger base flexion | No return for this type of data
    )
    JOINT_YAW_NR = (
        0x12  # Yaw angle - finger abduction/adduction | No return for this type of data
    )
    JOINT_ROLL_NR = (
        0x13  # Roll angle - only used for thumb | No return for this type of data
    )
    JOINT_TIP_NR = 0x14  # Fingertip angle control | No return for this type of data
    JOINT_SPEED_NR = (
        0x15  # Speed - motor running speed control | No return for this type of data
    )
    JOINT_CURRENT_NR = 0x16  # Current - motor running current feedback | No return for this type of data
    JOINT_FAULT_NR = (
        0x17  # Fault - motor running fault feedback | No return for this type of data
    )
...
class LinkerHandL20Can:
    def __init__(self, can_channel="can0", baudrate=1000000, can_id=0x28, yaml=""):
        ...
        self.xb0, self.xb1, self.xb2, self.xb3, self.xb4, self.xb5 = (
            [-1] * 5,
            [-1] * 5,
            [-1] * 5,
            [-1] * 5,
    ...
    def init_can_bus(self, channel, baudrate):
            ...
            if sys.platform == "linux":
                ...
                try:
                    ...
                    bus = can.interface.Bus(
                        channel=channel, interface="socketcan", bitrate=baudrate
                    )
                    ColorMsg(
                        msg=f"成功连接: interface='socketcan', channel='{channel}'",
                        color="green",
                    )
                    return bus
                ...
                try:
                    bus = can.interface.Bus(
                        channel=channel, interface="pcan", bitrate=baudrate
                    )
                    ColorMsg(
                        msg=f"成功连接: interface='pcan', channel='{channel}'",
                        color="green",
                    )
                    return bus
                except CanError as e:
                    ColorMsg(
                        msg=f"pcan 接口连接失败，尝试回退到 'candle': {e}",
                        color="yellow",
                    )
                ...
                try:
                    ...
                    ColorMsg(
                        msg=f"成功连接: interface='candle', channel='{channel}'",
                        color="green",
                    )
                    return bus
                except CanError as e:
        ...
        # --- 统一异常处理块结束 ---
            ...
            ColorMsg(
                msg=f"致命错误：所有 CAN 接口连接尝试均失败或平台不受支持。请检查设备连接或驱动安装和配置文件中CAN参数的配置。\n错误详情: {e}",
                color="red",
            )
    ...
    def send_command(self, frame_property, data_list, sleep=0.002):
        frame_property_value = (
            int(frame_property.value)
            if hasattr(frame_property, "value")
            else frame_property
        )
        data = [frame_property_value] + [int(val) for val in data_list]
    ...
    def pose_slice(self, p):
        ...
        try:
            ...
            thumb_yaw = [
                int(val) for val in p[10:15]
            ]  # Thumb yaw to palm, others are 0
    ...
    def get_touch(self):
        ...
        return [
            ...
            self.xb5[1],
            0,
        ]  # The last digit is palm, currently not available
